...
sudo pacman -Syu
sudo pacman -S python3 #or via AUR helpers such as yay or paru
sudo pacman -S nano
yay -S kitty #for yay AUR helper
paru -S kitty #for paru AUR helper
Then run via:
vault
The launch file will be stored in /usr/bin/vault so it can be launched without specifying the path. Until further notice, the file with the project has to be held in ~/VaultTUI.
## Usage guide:
